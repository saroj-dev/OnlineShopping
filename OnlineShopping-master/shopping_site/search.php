...
<?php
    if(isset($_GET['search_query'])){
        ...
        for($i = 0; $i < count($multiSearch); $i++){
            $searchQuery = "SELECT id FROM laptops WHERE laptopName LIKE '%$multiSearch[$i]%' OR laptopFeatures LIKE '%$multiSearch[$i]%' OR laptopPriceOrginal LIKE '%$multiSearch[$i]%' OR laptopRating LIKE '%$multiSearch[$i]%';";
        ...
        if(count($idOfItems) > 0){
            ...
            for($j = 0; $j < count($itemId); $j++){
                $getItemQuery = "SELECT * FROM laptops WHERE id = '$itemId[$j]'";
                ...
                if($itemResultRow > 0){
                    ...
                    echo '<div class="container">
                            <img src="img/'.$item["laptopFrontImage"].'" />
                            <div class="btm">
